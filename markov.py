...
# store and load its database files to. You probably want to give it
...
for i in range(3):
    ...
    while not goodSentence(mid, sentences[-1]):
        ...
        seedStr = random.choice(
            [z for z in sentences[-1].split() if z not in stopwords])
    sentences.append(mid)

sentences.append(mc_last.generateStringWithSeed(seedStr))
while not goodSentence(sentences[-1]):
    seedStr = random.choice(
        [z for z in sentences[-2].split() if z not in stopwords])
